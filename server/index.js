...
app.get('/', (req, res) => {
    //JavaScript ES6 표기법 (화살표 함수)
    res.send('Hello World! 뚱이 안녕 !');
});

// client에서 request 보낸 라우드 테스팅
...
app.post('/api/users/register', (req, res) => {
    ...
    user.save((err, userInfo) => {
        ...
        return res.status(200).send({
            //성공 시 (status 200 이 성공할 떄임 ) 제이슨 형식으로 success true를 반환
            success: true,
        });
    });
});
...
app.get('/api/users/auth', auth, (req, res) => {
    //auth : middleWare 이다. 콜백function 진행 전 처리해주는 로직
...
app.get('/api/users/logout', auth, (req, res) => {
    ...
    User.findOneAndUpdate(
        {_id: req.user._id}, //첫번째 파라미터 : 찾는 값, 두번째 파라미터: 변경할 값, 세번째 파라미터: 콜백함수
        {token: ''},
        (err, user) => {
            //만약 에러가 났다면
            if (err)
...
app.listen(port, () => {
    //5000번 port 에서 이 어플케이션을 실행 하는 것
